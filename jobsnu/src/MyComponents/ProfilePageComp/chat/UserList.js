...
function UserList(props) {
  ...
  const getUserChats = () => {
    Axios.get("./getUserChats?userId=" + userId).then(res => {
      //console.log(res.data)
      setChats(res.data);
    });
    Axios.get("./getUserName?userId=" + userId).then(res => {
      console.log(res.data);
      setName(res.data);
    });
  };
  ...
  const setOtherUserId = chat => {
    ...
    setCookie(
      "otherUserId",
      chat.member_user_ids.filter(e => e != cookies["userId"])[0]
    );
    props.setOtherUser(
      chat.member_user_ids.filter(e => e != cookies["userId"])[0]
    );
